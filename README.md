...
The FFmpeg (version 6.1) headers, minimally, are required if you are not also building FFmpeg. Said libraries can be linked statically or dynamically instead by defining `FFMPEG_DLL`. If dynamically linking, the libraries can be in their original separated forms or in a combined `ffmpeg.dll` if preferred. If statically linking, refer to the [FFmpeg documentation](https://ffmpeg.org/documentation.html) for building instructions and the necessary libraries to do so.  
